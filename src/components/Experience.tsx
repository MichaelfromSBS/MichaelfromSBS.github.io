      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-12">
          ...
          <p className="text-base text-slate-600 dark:text-slate-400 mt-2 max-w-2xl">
            My academic foundation at Carnegie Mellon University and organizational leadership roles.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            ...
            {educationData.map((edu, idx) => (
              <div
                key={idx}
                className="p-6 rounded-2xl bg-white dark:bg-slate-800/90 border border-slate-200/90 dark:border-slate-700/80 shadow-sm"
              >
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-700/60">
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {edu.coursework.map((course) => (
                        <span
                          key={course.code}
                          className="px-2 py-1 rounded text-xs bg-slate-100 dark:bg-slate-700/70 text-slate-800 dark:text-slate-200 border border-slate-200/60 dark:border-slate-600/50"
                          title={course.name}
                        >
              ...
              >
                ...
                <ul className="space-y-2 mb-4">
                  {exp.points.map((point, i) => (
                    <li key={i} className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 leading-relaxed list-disc list-inside">
                      <span>{point}</span>
                    </li>
                  ))}
                </ul>
